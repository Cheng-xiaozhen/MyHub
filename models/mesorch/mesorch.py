...
from ...common.backbones.extractors.high_frequency_feature_extraction import (
    HighDctFrequencyExtractor,
)
from ...common.backbones.extractors.low_frequency_feature_extraction import (
    LowDctFrequencyExtractor,
)
import math
...
class MixVisionTransformer(nn.Module):
    def __init__(
        ...
        if pretrain_path is not None:
            ...
            self.load_state_dict(torch.load(pretrain_path), strict=False)
...
@register_model("Mesorch")
class Mesorch(nn.Module):
    def __init__(
        self, seg_pretrain_path=None, conv_pretrain=False, if_predict_label=True
    ):
    ...
    def forward(self, image, *args, **kwargs):
        ...
        # 2. 通过head得到分类输出
        # features shape: [B, 1, H, W]
        out = self.head(features)  # out shape: [B, 1]
        out = out.squeeze(dim=1)  # out shape: [B]

        # 3. 从kwargs中获取label，并计算损失和预测值
        label = kwargs["label"]
