...
class EfficientNet(BaseModel):
    def __init__(
        ...
        self.model = timm.create_model(
            backbone, pretrained=pretrained, features_only=False
        )
        ...
        if output_type == "label":
            self.head = nn.Sequential(
                nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(out_channels, 1)
            )
            ...
            self.head = nn.Sequential(
                ...
                nn.Conv2d(
                    out_channels // 2, out_channels // 4, kernel_size=3, padding=1
                ),
                nn.ReLU(inplace=True),
        ...
        if input_head is not None:
            ...
            if num_channels > 0:
                new_first_layer.weight.data[:, 3:, :, :] = (
                    torch.nn.init.kaiming_normal_(new_first_layer.weight[:, 3:, :, :])
                )
...
class QtEfficientB4(EfficientNet):
    def __init__(self, output_type="label", pretrained=True, image_size=256):
        ...
        self.fusion_conv = nn.Conv2d(
            stem_out_channels + 256, stem_out_channels, kernel_size=1
        )
    ...
    def forward(self, image, dct, qt, *args, **kwargs):
        ...
        if fph_feat.shape[-2:] != x.shape[-2:]:
            fph_feat = F.interpolate(
                fph_feat, size=x.shape[-2:], mode="bilinear", align_corners=False
            )
